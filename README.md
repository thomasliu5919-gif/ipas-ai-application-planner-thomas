...
歷屆考題專區目前已依工作區下載的公告試題匯入 12 個官方題組，共 600 題，初級與中級分開管理；每一份歷屆試卷均為完整 50 題。若日後新增 PDF，可執行 `scripts/import-past-exams.py` 重新匯入。

歷屆考題專區採三層分類：先依初級／中級分開，再依年份，最後依梯次／次數展開各科目題組。
...
5. 正式歷屆題會依 `past-exams.js` 的 `questionIds` 對應完整題組；目前已匯入指定資料夾內的 12 份公告試卷，每份 50 題。
